...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame = None) -> Dict[str, Any]:
    ...
    # Новые эвристики (HW03) - только если df передан
    if df is not None:
        flags['has_constant_columns'] = any(df[col].nunique() == 1 for col in df.columns)
        flags['has_imbalanced_categories'] = any(
            df[col].dtype == 'object' and df[col].value_counts().iloc[0] > len(df) * 0.9
            for col in df.columns
        )
        flags['has_high_cardinality_categoricals'] = any(
            df[col].dtype == 'object' and df[col].nunique() > 100
            for col in df.columns
        )
        flags['has_suspicious_id_duplicates'] = 'id' in df.columns and df['id'].duplicated().any()
    ...
    if df is not None:
        if flags.get('has_constant_columns', False):
            score -= 0.1
        if flags.get('has_imbalanced_categories', False):
            score -= 0.1
...
def find_outliers(df: pd.DataFrame, threshold: float = 3.0) -> Dict[str, List[int]]:
    """
    Находит выбросы в числовых колонках.
    Возвращает словарь: колонка -> индексы строк с выбросами.
    """
    outliers = {}
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    
    for col in numeric_cols:
        # Используем z-score для определения выбросов
...
def get_problematic_columns(missing_df: pd.DataFrame, min_missing_share: float = 0.3) -> List[str]:
    """
    Возвращает список колонок с долей пропусков выше порога.
    """
    return missing_df[missing_df['missing_share'] > min_missing_share].index.tolist()
